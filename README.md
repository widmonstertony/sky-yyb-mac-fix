...
5. 以后在 M2/M4 上打开网易发烧游戏平台，再点《光·遇》的“开始游戏”即可；`launch.command` 会自动打开这条已验证链路。在已验证的 M4 环境中，启动台的独立《光·遇》图标也会直接打开或聚焦这个平台，不再调用会返回 `errCode:-1` 的应用宝子入口。游戏生成完整偏好文件后，安装脚本还会默认关闭游戏内全屏，让 Wine 窗口显示 macOS 标题栏；若首次安装时偏好尚未生成，退出游戏后重跑 `install.command`。
...
启动台里的《光·遇》入口会先确保网易登录会话存在，再通过官方 `fevergames://mygame/?gameId=63&autoRun=1` 协议让网易启动器拉起游戏；它不再直接运行 `Sky.exe`，也不绕过登录或反作弊。游戏首次生成完整偏好文件后，下一次启动会自动设为 60 FPS、关闭动态模糊并使用窗口化。Intel 实机验证：从原有入口重开后，左上角的 macOS 红色关闭、黄色最小化、绿色全屏按钮仍在。游戏会把窗口尺寸重置为较小的默认值；本修复保持的是窗口模式，不锁定窗口大小。
...
- 将光遇目标帧率设为 60，关闭动态模糊，并在游戏偏好中关闭全屏，保留 macOS 窗口标题栏与关闭/最小化/全屏按钮。Intel 已实机验证重启后保持；M2/M4 的窗口按钮效果尚待此版本实机复核。
...
### 光遇重开后没有 macOS 红黄绿窗口按钮

退出《光·遇》和网易平台，重跑 `install.command`。脚本会把游戏已生成的 `kUserPreference_Fullscreen` 偏好设为关闭；如果提示窗口偏好尚未生成，先在游戏图形设置里切换一次“窗口化”，正常退出，再重跑安装脚本。绿色按钮由 macOS 控制，仍可随时切入全屏；游戏重启后窗口大小可能恢复默认，但窗口化设置会保留。
