...
5. 以后在 M2/M4 上直接打开网易发烧游戏平台，再点《光·遇》的“开始游戏”即可；`launch.command` 会自动打开这条已验证链路。应用宝生成的独立《光·遇》图标在部分机器会返回 `errCode:-1`，因此不作为推荐入口。
...
这些 App 使用原生 macOS Dock 宿主：启动后会显示各自图标，按 `Command-Q` 或从 Dock 选择“退出”只会关闭对应的 Windows 启动器/游戏。已经运行的实例也可由同名 App 接管，不会重复启动。从网易启动器内部启动《光·遇》时，工具也会自动挂上独立的《光·遇》Dock 图标；游戏尚在运行时退出网易的 Dock 宿主会保留后台登录票据桥，避免 UniSDK 误报网络错误。
