...
3. Windows 版 Steam 和网易游戏启动器会安装到同一个应用宝 Wine 环境，并在 `/Applications` 生成可被 macOS 系统“应用”列表识别的 App。
4. 从 Steam 或网易启动器安装的游戏完成落盘后，后台同步会在 60 秒内把它生成为 `/Applications` 下的独立 App。
...
安装完成时，工具还会补齐旧启动器缺失的版本号和 `StartupPath`，使“扫码登录游戏”按钮能够正常拉起 `Sky.exe`。启动台里的《光·遇》入口会先确保网易登录会话存在，再以官方 `--start_from_launcher=1` 参数启动已安装的游戏；它不绕过登录或反作弊。游戏首次生成偏好文件后，下一次启动会自动设为 60 FPS 并关闭动态模糊。Intel 引擎冷启动时可能约有 15 秒没有窗口，请等待初始化完成。
...
- 将应用宝保存的发烧平台/光遇缩放比例修正为 Retina 2×，同步 MMKV CRC32，并修复腾讯 Intel 引擎错误回退到 1× 后备缓冲的路径。
- 补齐网易启动器游戏记录中的 `StartupPath` 和版本信息，使扫码登录完成后可以启动游戏。
- 将光遇目标帧率设为 60，关闭动态模糊。
...
关闭所有相关窗口后再运行一次 `install.command`。Intel 安装器会创建腾讯引擎需要的 MMKV Retina 2× 记录，并修补该版引擎错误回退到 1× 后备缓冲的问题；重跑是安全且幂等的。

### 扫码登录成功但游戏没有启动

先确认《光·遇》已经完整下载，再重跑一次 `install.command`。工具会补齐网易启动器缺失的 `StartupPath`；之后请从“网易游戏启动器”或 `/Applications/光·遇.app` 启动。Intel 引擎第一次冷启动可能需要约 15 秒。
