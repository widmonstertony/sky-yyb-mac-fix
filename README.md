...
该脚本只修改本机《剑星》与对应应用宝入口：开启 3840×2160 物理输出、Retina 2×、FSR 3 质量超分与帧插值，将帧率上限设为 120，并把画质预算优先分配给人物高细节/超高纹理。景深、动态模糊、色差和颗粒会关闭，环境项保持中低以适配 16 GB 统一内存。它还会绕过容易卡住的启动影片等待，并保留 Steam 启动日志。

这里的 4K 是最终输出分辨率；FSR 质量模式会以较低内部像素数重建 4K，从而争取稳定 60 FPS 以上。实际帧率仍随场景、温度和后台负载变化。运行前会停止应用宝 Wine 会话并备份配置；光遇的 Vulkan 补丁和存档不会被修改。
...
这些 App 使用原生 macOS Dock 宿主：启动后会显示各自图标，按 `Command-Q` 或从 Dock 选择“退出”只会关闭对应的 Windows 启动器/游戏。已经运行的实例也可由同名 App 接管，不会重复启动。从网易启动器内部启动《光·遇》时，工具也会自动挂上独立的《光·遇》Dock 图标；游戏尚在运行时会阻止网易 Dock 宿主退出并给出提示，既保留登录票据桥，也不会留下没有 Dock 图标的后台启动器。
